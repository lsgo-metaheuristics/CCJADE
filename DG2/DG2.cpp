...
DG2::DG2 (Fitness* f, unsigned _dim, tFitness _lb, tFitness _ub): g(_dim-1), dim(_dim), lb(_lb), ub(_ub)
{  
    this->func = f;    
	...
	eInfV = new double*[dim];
	eSupV = new double*[dim];
    ...
    for (int i = 0; i < dim; i++){
		...
		eInfV[i] = new double[dim];
		eSupV[i] = new double[dim];
...
std::vector< std::vector<double> > DG2::getLambda()
{
	std::vector<std::vector<double>> v(dim);
	for (int i = 0; i < dim; i++) {
		for (int j = 0; j < dim; j++) {
			v[i].push_back(lambda[i][j]);
		}
	}
	return v;
}

std::vector< std::vector<double> > DG2::getEInf()
{
	std::vector<std::vector<double>> v(dim);
	for (int i = 0; i < dim; i++) {
		for (int j = 0; j < dim; j++) {
			v[i].push_back(eInfV[i][j]);
		}
	}
	return v;
}
...
std::vector< std::vector<double> > DG2::getESup()
{
	std::vector<std::vector<double>> v(dim);
	for (int i = 0; i < dim; i++) {
		for (int j = 0; j < dim; j++) {
			v[i].push_back(eSupV[i][j]);
		}
	}
	return v;
}
    ...
    for (int i = 0; i < dim - 1; i++){
        ...
        for (int j = i + 1; j < dim; j++){
            ...
            if (prog % 5 == 0 && prev != prog){
                printf("DG2 progress = %02d\r", prog);
                std::cout.flush();
            }
...
void DG2::dsm (void){
    ...
    for (int i = 0 ; i < dim - 1; i++){
        for (int j = i + 1; j < dim; j++){
            ...
            //eSup = gammaFunc(std::sqrt(dim)) * fMax;
            eSup = gammaFunc(pow((double)dim, (double)0.5)) * fMax;
			eSupV[i][j] = eSupV[j][i] = eSup;
			eInfV[i][j] = eInfV[j][i] = eInf;
			if (lambda[i][j] <= eInf){
                theta[i][j] = 0;
                theta[j][i] = 0;
                etha0 += 1;
