...
namespace CEC2010 {
  ...
  unsigned Benchmarks::getDimension()
	...
	double Benchmarks::nextDouble(){
		return ((((_L(next(26))) <<
			(_L(27))) + (_L(next(27)))) / (double)((_L(1)) << (_L(53))));
	}
	...
	double Benchmarks::nextGaussian(){
		...
		do {
			v1 = ((_D(2.0) * nextDouble()) - _D(1.0));
			v2 = ((_D(2.0) * nextDouble()) - _D(1.0));
			s = ((v1 * v1) + (v2 * v2));
		} while ((s >= _D(1.0)) || (s <= _D(0.0)));

		multiplier = sqrt(_D(-2.0) * log(s) / s);
	...
	double* Benchmarks::createShiftVector(int dim, double min, double max) {
		...
		hw = (_D(0.5) * (max - min));
		middle = (min + hw);
		//_D      = (double*)malloc(sizeof(double) * dim);
		d = new double[dim];
	...
	double Benchmarks::rastrigin(double*x, int dim){
		...
		for (i = dim - 1; i >= 0; i--) {
			sum += x[i] * x[i] - 10.0 * cos(2 * _PI * x[i]) + 10.0;
		}
	...
	double Benchmarks::rastrigin(double *x, int dim, int k)
	{
		...
		for (i = dim / k - 1; i >= 0; i--)
		{
			result += x[Pvector[dim / k + i]] * x[Pvector[dim / k + i]] - 10.0*cos(2 * _PI*x[Pvector[dim / k + i]]) + 10.0;
		}
		return(result);
	}
	...
	double Benchmarks::ackley(double*x, int dim){
		...
		for (i = dim - 1; i >= 0; i--) {
			sum1 += (x[i] * x[i]);
			sum2 += cos(2.0 * _PI * x[i]);
		}

		sum = -20.0 * exp(-0.2 * sqrt(sum1 / dim)) - exp(sum2 / dim) + 20.0 + _E;
		return(sum);
	}
	...
	double Benchmarks::ackley(double *x, int dim, int k)
	{
		...
		for (i = dim / k - 1; i >= 0; i--)
		{
			...
			sum2 += cos(2.0*_PI*x[Pvector[dim / k + i]]);
		}

		result = -20.0*exp(-0.2*sqrt(sum1 / (dim / k))) - exp(sum2 / (dim / k)) + 20.0 + _E;
	...
	double Benchmarks::rot_rastrigin(double *x, int dim, int k)
	{
		...
		for (i = dim - 1; i >= 0; i--)
		{
			result += anotherz1[i] * anotherz1[i] - 10.0*cos(2 * _PI*anotherz1[i]) + 10.0;
		}
		return(result);
	}
	...
	double Benchmarks::rot_ackley(double *x, int dim, int k)
	{
		...
		for (i = dim - 1; i >= 0; i--)
		{
			...
			sum2 += cos(2.0*_PI*anotherz1[i]);
		}
		result = -20.0*exp(-0.2*sqrt(sum1 / dim)) - exp(sum2 / dim) + 20.0 + _E;
		return(result);
	}
